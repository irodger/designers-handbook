...
Это можно отнести ко всему, но в нашем случае это относится как к отношениям между `designer <> developer` и `designer <> user`. Когда ты оставляешь на "и так сойдет", этот секс скатывается на плечи тех, кто взаимодействует с твоей работой. Юзеру приходится гадать как тот или иной элемент работает и почему оно сделано так. Разработчику приходится гадать как эти же элементы должны были работать по твоей задумке и делать за тебя работу, вместо своей. __Это плохо__.
...
> Я никогда не любил математику, да, мы существуем, разработчики, не любящие математику.  
Дело в том, что размер окна браузера четный, да, ты можешь повозюкать браузер за край, молодец, но я верстаю все равно используя какое-то дефолтное разрешение, потом уже накручивая адаптив. 
