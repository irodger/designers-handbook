...
* [Глава 5. Состояния (WiP)](#глава-5-состояния-интерактивных-элементов)
...
Зачем-то придумали, чтоб у каждого слоя было какое-то осмысленное название, например компонент шапка мог бы называться Header, а не слой 128.  
Понимаю твое недоразумение, когда тебя просят уделить этому внимание. Ты смотришь на макет, для тебя это и есть результат. Для разработчика твой макет это не просто набор графики, это источник данных для реализации. Размеры, отступы, ширина, цвет, все это ему нужно находить. Поверь, это гораздо проще делать, когда все подписано.

---

## Глава 3. SVG
Суй в контейнеры, следи за полупикселем.
SVG - прекрасный формат векторных изображений, давай держать это в голове. Нет, растровая картинка, засунутая в векторный контейнер не становится векторной - не делай так.  
  
Если работаешь в Figma и любишь с градиентами побаловаться, в Figma есть нюанс - градиент в Figma накладывается маской, через id svg элемента. Figma не очень замарачивалась генерировать рандомные айдишники для svg масок и это все приводит к ситуации, где на веб странице у всех svg элементов с градиентной маской один и тот же градиент, если так задуманно, то ок. Однажды я написал плагин для Figma, который позволял сгенерировать рандомный айдишник для svg кода, но к сожалению идея была обречена на провал, т.к. плагины в figma могут использовать пользователи с ролью дизайнер, разработчикам недоступны. Возможно, что Figma уже решили этот нюанс.  

Внутри svg-контейнера элементы находятся в своей плоскости и для них нецелое количество пикселов является нормой, за пределом конейтнера - нет. Следи за тем, чтоб контейнер имел целое значение в пикселах. Идеально, если размер контейнера сможет вписаться в 8-пксельную сетку (т.е. при делении на 8, будет делиться без остатка, на крайняк можно поделить на 4)
...
## Глава 5. Состояния интерактивных элементов
Не забывай! Особенно про фокус!
Когда речь заходит о состояниях элементов в дизайне, у разработчика с дизайнером начинаются расхождения в понимании и вот почему -
Для разработчика состояния - это набор определенных свойств интерактивных элементов, нет шанса перепутать ховер с фокусом.

Для дизайнера - это абстрактный набор, который он эмулирует в дизайне, создавая видимость, что элемент находится в каком-то состоянии. Отсюда можно увидеть, что дизайнер знает о состояниях элементов в теории.
Давай пройдемся по состояниям, которые знает разработчик:
- `default` - Дефолтное состояние, когда элемент находится в своем обычном состоянии.
- `hover` - Состояние, когда пользователь навел указатель мышки на элемент. Его нет в мобилках(без подключения мышек, считай нет), иногда айфоны ловят наши ховеры при тапе по элементу, передай разработчику, чтоб "зафолбэчил ховер в css"
- `active` - Состояние, когда на элемент нажали мышкой или зажали тап, в этот момент элемент получает состояние active. Это не про выбранный таб, т.к. после того как клик происходит, элемент теряет состояние active
- `disabled` - Состояние, когда элемент нельзя выбрать или взаимодействовать с ним, например поле ввода, в которое нельзя ввести
- `focus` - Фокус-покус - состояние, когда элемент находится во внимании, в фокусе и это не про наведение мышкой. Элемент может оказаться в фокусе, если - на нем произошел клик/тап, например установлен курсор в поле ввода, или, к примеру, кнопкой tab можно перемещаться к следующему интерактивному элементу на сайте, чтоб понимать какой сейчас элемент выбран, он переходит в состояние focus, которое, в идеале, тоже прорисовывают
...
### Делай прототипы
Можно собрать 3 созвона, где на словах объяснить всем участникам как все должно работать вместе, потом еще 7 раз ответить в чате про то, что из чего появляется или сделать интерактивный прототип из нарисованных макетов и один созвон, где показать, что он есть и его можно использовать. 100 раз услышать и 1 раз увидеть это про этот момент.
